                                                ...
                                                @if(!empty($f->image) && strlen($f->image) > 5 && Storage::disk('s3')->exists($f->image))
                                                    <article class="card mb-4">
                                                        ...
                                                        <div class="card-body">
                                                            ...
                                                            @endif
                                                            <div>
                                                                <a href="@if(!empty($route)){{ $route.'/'.$f->slug }}@else{{ route($app->module.'.show', $f->slug) }}@endif" class="btn btn-sm btn-primary">@if($settings->language == 'telugu') మరింత సమాచారం @else Continue Reading @endif</a>
                                                            </div>
                                                        </div>
                                                    </article>
                                                @else
                                        ...
                                        @if(!empty($f->image) && strlen($f->image) > 5 && Storage::disk('s3')->exists($f->image))
                                            <div style="max-height: 13.5rem;overflow: hidden;">
                                                @php
                                                    ...
                                                    $path = $path[0];
                                                @endphp
                                                <img class="card-img-top img-fluid rounded-lg" src="{{ Storage::disk('s3')->url('resized_images/'.$path.'_mobile.'.$ext) }}">
                                            </div>
                                        @endif
                                                    ...
                                                    @if($k == 0)
                                                        @if(!empty($obj->image) && strlen($obj->image) > 5 && Storage::disk('s3')->exists($obj->image))
                                                            ...
                                                            <div class="card transition-3d-hover">
                                                                ...
                                                                @endif
                                                                <div class="card-body">
                                                                    <h5><a class="text-decoration-none text-dark" href="@if(!empty($route)){{ $route.'/'.$obj->slug }}@else{{ route($app->module.'.show', $obj->slug) }}@endif">{{$obj->title}}</a></h5>
                                                                    <div class="mb-3">
                                                                        @if(!empty($tags))
                                                                            @foreach($tags as $tag)
                                                                                <a href="{{ route('Tag.show', $tag->slug) }}" class="badge rounded-badge bg-soft-primary px-2 py-1 text-primary">{{ $tag->name }}</a>
                                        ...
                                        <div class="col-6 p-0">
                                            <ul class="pl-4">
                                                @foreach($category->posts->take(5) as $k => $obj)
                                                    @if($obj->status != 0)
                                                        @if($k != 0)
                                                            <li class="my-3 h5 text-dark"><a href="@if(!empty($route)){{ $route.'/'.$obj->slug }}@else{{ route($app->module.'.show', $obj->slug) }}@endif" class="text-muted" >{{ $obj->title }}</a></li>
                                                        @endif
                                                    @endif
                                                @endforeach
                                            </ul>
